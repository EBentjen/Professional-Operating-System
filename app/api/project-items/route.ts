...
export async function POST(req: NextRequest) {
  const db = getDb();
  const { project_id, title, order_index = 0 } = await req.json();
  if (!project_id || !title?.trim()) return NextResponse.json({ error: 'project_id and title required' }, { status: 400 });
  const result = db.prepare(
    'INSERT INTO project_items (project_id, title, order_index) VALUES (?, ?, ?)'
  ).run(project_id, title.trim(), order_index);
...
export async function PATCH(req: NextRequest) {
  ...
  const allowed = ['title', 'is_complete', 'order_index'];
